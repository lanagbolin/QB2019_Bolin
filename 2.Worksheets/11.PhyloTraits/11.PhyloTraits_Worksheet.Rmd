...
rm(list = ls())
getwd()
setwd("/Users/lana/GitHub/QB2019_Bolin/2.Worksheets/11.PhyloTraits")
...
> ***Answer 1***: The .afa file has deletions marked as dashes, while the .fasta file doesn't. 
...
> ***Answer 2a***: ~400 bp
> ***Answer 2b***: Regions that show a lot of varation among isolates are appropriate because that allows those regions to be grouped in a meaninful way (in other words, if we chose regions without much variation our tree would have lots of unhelpful polytomies).
...
> ***Answer 3***:  It's fast so it's a great place to start looking at your data, but it's not as sophistocated as some other types of tree. For example, it doesn't take into account basepair transition probabilities.
...
par(mar = c(1, 1, 2, 0))
plot.phylo(raw.rooted, type = "phylogram", direction = "right", show.node.label = TRUE, use.edge.length = FALSE, adj = 0.5, cex = 0.6, label.offset = 2, main = "Raw")

par(mar = c(1, 0, 2, 1))
plot.phylo(F84.rooted, type = "phylogram", direction = "left", show.node.label = TRUE, use.edge.length = FALSE, adj = 0.5, cex = 0.6, label.offset = 2, main = "F84")
```
...
plot(seq.dist.T92, seq.dist.F84,
     pch = 20, col = "red", las = 1, asp = 1, xlim = c(0, 0.7), ylim = c(0, 0.7), xlab = "T92 Distance", ylab = "F84 Distance")
abline(b = 1, a = 0, lty = 2)
text(0.65, 0.6, "1:1")
...
layout(matrix(c(1, 2), 1, 2), width = c(1, 1))
par(mar = c(1, 1, 2, 0))
plot.phylo(T92.rooted, type = "phylogram", direction = "right", show.node.label = TRUE, use.edge.length = FALSE, adj = 0.5, cex = 0.6, label.offset = 2, main = "T92")
...
> ***Answer 4a***:   I chose the Tamura model (T92), which assumes equal frequencies of nucleotides but recognizes that transition mutations occur with higher probability than transversion mutatuions, and accounts for G + C content of the sequences.
> ***Answer 4b***:   T92 and F84 models create the same phylogeny. Their saturation plot has points along the 1:1 line, and the trees match perfectly.
> ***Answer 4c***:   Because these models create identical trees, substitution rates of nucleotide transitions are not different enough to change the tree structure.
...
> ***Answer 5a***:   It's quite different from the neighbor-joining tree, presumably because it takes into acccount nucleotide states? 
> ***Answer 5b***:   Bootstrapping lets us run stats on our and generate confidence intervals.
> ***Answer 5c***:   They tell you in what proportion of your bootstrapped trees did a particular node/branch show up. A high value indicates the node/branch is well supported.
> ***Answer 5d***:   Many of the branches in the clade that includes WG42, LL43F, WG26, LLX3, LLX12A, and LL1 have very low support. 
...
# 2
outgroup <- match("Methanosarcina", nj.tree$tip.label)
nj.rooted <- root(nj.rooted, outgroup, resolve.root = TRUE)
...
table.phylo4d(x, treetype = "phylo", symbol = "colors", show.node = TRUE, cex.label = 0.5, scale = F, use.edge.length = F, edge.color = "black", edge.width = 2, box = F, col = mypalette(25), pch = 15, cex.symbol = 1.25, ratio.tree = 0.5, cex.legend = 1.5, center = F)

# 3
par(mar = c(1, 5, 1, 5) + 0.1)
x.nb <- phylo4d(nj.rooted, nb)

table.phylo4d(x.nb, treetype = "phylo", symbol = "colors", show.node = TRUE, cex.label = 0.5, scale = F, use.edge.length = F, edge.color = "black", edge.width = 2, box = F, col = mypalette(25), pch = 15, cex.symbol = 1.25, var.label = "       NB", ratio.tree = 0.9, cex.legend = 1.5, center = F)
```
...
> ***Answer 6a***:   Species that can grow well on many resources have a low maximum growth rate.
> ***Answer 6b***:   Species with a high niche breadth have a low umax, while species that have a low niche breadth have a high umax.
...
> ***Answer 7a***:  Untransformed lambda = 0.062, transformed lambda = 0
> ***Answer 7b***:  The difference in AIC is about 0.12, so the models are about equivalent.
> ***Answer 7c***:  Nope.
...
rownames(p.phylosignal) <- c("k", "PIC.vars.obs", "PIC, var.mean", "PIC.var.P", "PIC.var.z", "PIC.P.BH")

for (i in 1:18) {      # Thist calculates Bloomberg's k for each resource
  x <- as.matrix(p.growth.std[, i, drop = FALSE])   # store column "x" as matrix
  out <- phylosignal(x, nj.rooted)  # compare column "x" to rooted tree
  p.phylosignal[1:5, i] <- round(t(out), 3)  # Round 
}
...
> ***Answer 8a***:   There is no significant phylogenetic signal for niche breadth. There is phylogenetic signal for cAMP and DNA (p<0.05)
> ***Answer 8b***:   k=0, so results suggest overdispersion.
...
> ***Answer 9a***:  DNA and cAMP are significantly overdispersed, and ATP is marginally overdispersed. 
> ***Answer 9b***:  These are consistent with Blomberg's K analysis.
> ***Answer 9c***:  When we calculate D for each metric we aren't correcting for multiple comparisons, so we might expect to see more significantly clumped or overdipsersed traits using D than K.
...
> ***Answer 10a***:  Species on our tree aren't independent - they share evolutionary history. So we need to account for that in our model.
> ***Answer 10b***:  The variance of the error term in a phylogenetic regression is described by a covariance matrix that takes into account branch lenghts. In a standard linear regression the variance of the error term is a constant (sigma squared).
> ***Answer 10c***:  The slope of the model that accounts for phylogeny has a higher slope than the simple regression model, meaning the relationship between mass and BMR has a higher slope than we'd think if we didn't account for shared ancestry. The AIC value for model that accounts for phylogeny is much lower than the simple regression model, so it's a better fit. 
> ***Answer 10d***:  Say you wanted to know how shouting affected plant growth rate, so you had a shouting treatment with 10 tree species and a no-shouting treatment with 10 grass species. At the end of the experiment you found that the shouting treatment had a higher growth rate! But of course that's just because they're all trees, and trees grow faster than grasses. So once you account for shared ancestry, there wouldn't be any effect of shouting (probably...). That's my best shot!
...
```{r}
# Visualize
read.aln <- read.alignment(file = "./data/plant_seq.afa", format = "fasta")
p.DNAbin <- as.DNAbin(read.aln)
window <- p.DNAbin[, 100:500]
image.DNAbin(window, cex.lab = 0.5)
...
seq.dist.F84 <-dist.dna(p.DNAbin, model = "F84", pairwise.deletion = FALSE)
F84.tree <-bionj(seq.dist.F84)
F84.outgroup <-match("Calamagrostis", F84.tree$tip.label)
F84.rooted <-root(F84.tree, F84.outgroup, resolve.root = TRUE)
...
> Yep, the tree jibes with what is known! Rudbeckia, Silphium, Vernonia, and Solidago are all Asteracaea, so it makes sense that they form a clade. We chose two species from each genus, and the species grouped together by their genera. Hooray! 
